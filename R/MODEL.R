source("R/utils/loadpackages.R")# also load modules
my.year=2015
model_res = 10 # standard is 10 no change
time_step <- 7
...
nReplicate <- 3 # how many replicates of the population simulation
nYearSim <- 1 # how many years of simulation pour le moment une seule. réinitialisé à chaque année.
# search.mec = c("IFD", "IFD.dd","GAS", "GAS.dd")

start_date <- ymd(paste0(my.year, "-01-01")) ## for now use the same start date as data otherwise subset of raster not working
end_date <- ymd(paste0(my.year, "-12-30")) #
prob_moving <- 0 #0.1=PROBABILITY OF MOVING OF 90%

param <- paramInit(time_step = time_step)
d.d. <- T
search.mec <- c("IFD.dd")
zoo.var= "zooplankton"
temp.var= "sst" ## sst plus chaud permet d'avoir une migration plus tôt

# https://github.com/SarahBauduin/appendix_wolfIBM/blob/master/R/run.R#L71
# Create a df to record the dynamic
fishDyn <- list()
# repSim = rep(1:nReplicate, each = nYearSim),#inlude replicates and year,  but for a loop with year need to initialize fish.
# yearSim = rep(my.year, nReplicate),
# # year = my.year,
#  search.mec = search.mec
# ) # number of packs created during the the year by individuals alone
...
fish<- breed_transformation(fish=fish, time_step=time_step)
  ...
  radius1 <- inRadius(agents = fish, radius = length_to_distance(fish, param = paramInit(time_step = time_step), grid.resolution=model_res) * time_step, agents2 = patches(seaworld))
  radiusdd <- inRadius(agents = fish, radius = length_to_distance(fish, param = paramInit(time_step = time_step), grid.resolution=model_res) * time_step, agents2 = fish) # density dependance
...
if(barrier) source("R/modules/avoid_land.R")
  ...
  fish.df1 <- larvae_feeding(fish = turtle(fish, who=fish$who, breed="larvae"), seaworld = seaworld, d.d. = d.d., param = paramInit(time_step = time_step), temp.var = temp.var, zoo.var = zoo.var)
  fish.df2 <- adult_feeding(fish = turtle(fish, who=fish$who, breed="adult"), seaworld = seaworld, d.d. = d.d., param = paramInit(time_step = time_step), temp.var = temp.var, zoo.var = zoo.var)
  if(nrow(fish.df2) >0) fish.df2 <- spawning(fish.df2)
  fish.df3 <- adult_feeding(fish = turtle(fish, who=fish$who, breed="juvenile"), seaworld = seaworld, d.d. = d.d., param = paramInit(time_step = time_step), temp.var = temp.var, zoo.var = zoo.var)
  ...
  eggs<- fish.df %>%  filter(state==3) %>%  group_by(who, xcor,ycor) %>%  
    summarize(N=sum(realised.fecundity)*0.5) %>% ungroup() 
...
# if(t ==22) stop()#Sys.Sleep(100)
  
  #return(popSim)
}

write_rds(popSim, paste0("results/popSim_",model_res,"km_",step.name, my.year, "_v1.RDS"))
write_rds(fishDyn, paste0("results/fishDyn_",model_res,"km_",step.name, my.year, "_v1.RDS"))
